...
public class SwiftFlutterTtsPlugin: NSObject, FlutterPlugin, AVSpeechSynthesizerDelegate, @unchecked Sendable {
        ...
        let didWrite = try context.write(pcmBuffer) { temporaryURL in
          ...
          if #available(iOS 17.0, *) {
            guard let audioFormat = AVAudioFormat(
              ...
              throw NSError(
                domain: "flutter_tts.synthesis",
                code: 1,
                userInfo: [NSLocalizedDescriptionKey: "Unable to create the synthesis audio format"]
              )
            }
            return try AVAudioFile(forWriting: temporaryURL, settings: audioFormat.settings)
          }
  ...
  private func deactivateAudioSessionIfNeeded() {
    guard shouldDeactivateAndNotifyOthers(audioSession) && self.autoStopSharedSession else {
      return
    }
    do {
  ...
  private func dispatchSpeechDelegateCallback(
    _ callback: @escaping @Sendable (SwiftFlutterTtsPlugin) -> Void
  ) {
  ...
  private func handleSpeechDidFinish(_ utteranceKey: ObjectIdentifier) {
    ...
    if self.speechUtteranceContexts.isEmpty {
      deactivateAudioSessionIfNeeded()
    }
...
#if SWIFT_PACKAGE
/// Swift Package Manager entry point. CocoaPods uses the Objective-C wrapper
/// with the same class name because SwiftPM doesn't support mixed-language targets.
public final class FlutterTtsPlugin: NSObject, FlutterPlugin {
  public static func register(with registrar: FlutterPluginRegistrar) {
